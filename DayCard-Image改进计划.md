...
| API 功能 | 模型列表为静态硬编码，无法感知服务端实际可用模型 | 待处理（阶段二 2.1） |
| API 功能 | `testConnection` 仅探活，无返回模型列表、配额等信息 | 待处理（阶段二 2.3） |
| ~~API 功能~~ | ~~OpenAI Provider 的 `dailyUsed` 存内存，重启归零，配额不准确~~ | ✅ 阶段一 1.5 已修复（T-108，storageAdapter 持久化） |
| API 功能 | 不支持 API base URL 自定义（代理 / 私有部署场景） | 待处理（阶段二 2.2，注册表已预留 defaultBaseURL 字段） |
...
| 二：API 能力 | 第 3–4 周 | 动态模型列表、Base URL 自定义、增强连接测试 | 待启动 |
