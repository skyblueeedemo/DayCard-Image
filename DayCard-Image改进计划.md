...
> 编写日期：2026-05-17  
> 最后更新：2026-05-17（阶段四打磨发布已交付，v1.4.0 发版）
...
**阶段一基础重构已交付**（2026-05-17，未发版，等阶段二一起到 dev1.4.0）：
- ✅ 1.1 路由系统重构：`src/router/routes.ts` + App + Sidebar 接入 ROUTES 声明式渲染
- ✅ 1.2 消除 generate / retryGenerate 重复：提取 `doGenerate` 私有函数，净减 22 行
- ✅ 1.3 统一存储层：`src/store/storageAdapter.ts` + 13 条单元测试，renderer 8 处分散调用全部接入
- ✅ 1.4 Provider 注册表元数据化：`src/providers/registry.ts` 集中管理，ApiConfigPage 删除 25 行硬编码
- ✅ 1.5 OpenAI 配额持久化：通过 storageAdapter 写入 `daycard-quota-openai`，跨重启保留
- ✅ 顺手清理：MockProvider 测试 10% 概率失败，已用确定性 stub 修复
- 全量测试 30 → 44 用例，type-check + lint 全绿

**阶段二 API 能力升级已交付**（2026-05-17，未发版，按用户决策不打 tag）：
- ✅ 2.1 动态模型列表：5 个 Provider 实现 listModels（OpenAI/Stability 走 API，Zhipu/Aliyun 失败 fallback，Mock 静态）；renderer 用 storageAdapter 缓存 10 分钟
- ✅ 2.2 Base URL 自定义：ProviderConfig.baseURL 持久化，generate / list-models / test 三条链路全部生效，UI 折叠输入框
- ✅ 2.3 增强连接测试：testConnection 返回 latencyMs + errorCode；补全 stability / zhipu 探活路径
- ✅ 2.4 OpenAI 模型扩展：GenerateOptions.model 透传，Web 模式同样支持
- ✅ 2.5 IPC 配额持久化（阶段一 T-108 已交付）
- ✅ 新增 listModels 单元测试 10 条，测试套件 44 → 54

**阶段四打磨发布已交付**（2026-05-17，**v1.4.0 发版**）：
- ✅ 4.1 键盘快捷键扩展：Ctrl/Cmd + , 跳设置 / Ctrl/Cmd + 1~6 切页（通过自定义事件解耦）
- ✅ 4.2 ErrorBoundary 增强：错误概要卡片 + 时间戳 + GitHub Issues 报告问题入口
- ✅ 4.3 性能优化：ImageCard React.memo + ProviderSelector refresh 300ms debounce + loadConfig 5 秒缓存
- ⚠️ 4.4 自动更新 UI：按 D-4.1=A 决策跳过（现状是 toast 降级提示）
- ⚠️ 4.5 README 截图：按 D-4.2=A 决策由用户自补
- ✅ dev/main 双分支工作流：日常推 dev，稳定快照合并 main 打 tag
- ✅ package.json + UI 升至 v1.4.0；阶段一~四 53 个 commit 统一发版

至此改进计划阶段一~四全部交付。3-C 第二轮（主页 Hero / Masonry / 吸附工具栏 / 悬浮操作层）作为可选项保留，后续按需启动。
...
| 类别 | 问题描述 | 状态 |
|------|---------|------|
| UI 高级感 | Sidebar 使用 emoji 图标 + 纯灰底色，视觉层次平 | 待处理（阶段三） |
| UI 高级感 | 主页布局为单列居中堆叠，空间利用率低，缺乏视觉焦点 | 待处理（阶段三） |
| UI 高级感 | ImageCard 无骨架屏，加载态仅 spinner，体验断层 | 待处理（阶段三） |
| UI 高级感 | 色彩体系仅 gray + blue，品牌色缺失，暗色模式对比度弱 | 待处理（阶段三） |
| ~~框架/结构~~ | ~~`App.tsx` 路由逻辑用字符串联合类型 + `if/else`，不可扩展~~ | ✅ 阶段一 1.1 已修复（T-105/T-106） |
| ~~框架/结构~~ | ~~`ApiConfigPage` 中硬编码 `DEFAULT_MODELS`，新增 Provider 需改多处~~ | ✅ 阶段一 1.4 已修复（T-103/T-104） |
| ~~框架/结构~~ | ~~Provider 排序存 `localStorage`，Electron 下与 `userData` 存储双写不一致~~ | ✅ 阶段一 1.3 已修复（renderer 端收敛到 storageAdapter；Electron userData 仍由 IPC 处理，两者职责清晰） |
| ~~框架/结构~~ | ~~`generationStore.retryGenerate` 与 `generate` 代码高度重复~~ | ✅ 阶段一 1.2 已修复（T-107，提取 doGenerate） |
| ~~API 功能~~ | ~~模型列表为静态硬编码，无法感知服务端实际可用模型~~ | ✅ 阶段二 2.1 已修复（T-201/T-203/T-206） |
| ~~API 功能~~ | ~~`testConnection` 仅探活，无返回模型列表、配额等信息~~ | ✅ 阶段二 2.3 已修复（T-202，返回 latencyMs + errorCode） |
| ~~API 功能~~ | ~~OpenAI Provider 的 `dailyUsed` 存内存，重启归零，配额不准确~~ | ✅ 阶段一 1.5 已修复（T-108，storageAdapter 持久化） |
| ~~API 功能~~ | ~~不支持 API base URL 自定义（代理 / 私有部署场景）~~ | ✅ 阶段二 2.2 已修复（T-204/T-205/T-206） |
...
## 阶段一：基础重构与代码质量（2 周） ✅ 已交付（2026-05-17）

**目标**：清除技术债，为后续功能扩展打稳地基，不破坏现有功能。

> 全部子项已完成。详细任务记录见 `tasks.md` 优化阶段 3 一节。
> Commits：14e4007 / 0b131b7 / 2a3b893 / 5daf163 / fad87bc / d755ba7 / 42ddc47 / 1f2ef8e / 0193ccf
> 测试套件 30 → 44，type-check 0 errors，lint 0 errors
...
## 阶段二：API 能力升级（2 周） ✅ 已交付（2026-05-17）

**目标**：模型列表动态获取、Base URL 自定义、连接测试增强。

> 全部子项已完成。详细任务记录见 `tasks.md` 优化阶段 4 一节。
> Commits：831cffa / dd3324d / 8be2aca / eee0a9c / 9608cdb / f69a5ad / aba7252 / 4fdd0e4
> 测试套件 44 → 54，type-check + lint 全绿
...
## 阶段四：体验打磨与发布准备（1 周） ✅ 已交付（2026-05-17，v1.4.0 发版）

**目标**：细节收尾，准备 v2.0.0 发布。

> 4.1 / 4.2 / 4.3 已落地；4.4 / 4.5 按决策跳过。详细任务记录见 `tasks.md` 优化阶段 5 一节。
> Commits（dev 分支）：d452662 / 7aaba9c / ef26e9c / 933f044 / f244563 + 升版 commit
...
| 阶段 | 周期 | 核心产出 | 状态 |
|------|------|---------|------|
| 一：基础重构 | 第 1–2 周 | 路由表化、存储统一、Provider 注册表、配额持久化 | ✅ 已交付（2026-05-17） |
| 二：API 能力 | 第 3–4 周 | 动态模型列表、Base URL 自定义、增强连接测试 | ✅ 已交付（2026-05-17） |
| 三：UI 升级 | 第 5–7 周 | 设计 Token、Sidebar、主页布局、ImageCard、动画 | ✅ 第一轮已交付（2026-05-17）；第二轮待启动 |
| 四：打磨发布 | 第 8 周 | 快捷键、错误处理、性能、自动更新 UI、发布 | ✅ 已交付（2026-05-17，发版 v1.4.0） |
